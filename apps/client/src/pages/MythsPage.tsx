...
const RARITY_CONFIG: Record<Rarity, {
  label: string; border: string; glow: string; badge: string; text: string; glowRgb: string;
}> = {
  COMMON:    { label: "Común",      border: "border-[#5a6a85]",  glow: "shadow-[0_0_12px_rgba(90,106,133,0.4)]",   badge: "bg-[#1e2d45] text-[#8ca0b8]",  text: "text-[#8ca0b8]",  glowRgb: "90,106,133"   },
  RARE:      { label: "Raro",       border: "border-[#4cc9f0]",  glow: "shadow-[0_0_14px_rgba(76,201,240,0.4)]",   badge: "bg-[#0a2233] text-[#4cc9f0]",  text: "text-[#4cc9f0]",  glowRgb: "76,201,240"   },
  ELITE:     { label: "Élite",      border: "border-[#9b5de5]",  glow: "shadow-[0_0_16px_rgba(155,93,229,0.45)]",  badge: "bg-[#1a0b33] text-[#c77dff]",  text: "text-[#c77dff]",  glowRgb: "155,93,229"   },
  LEGENDARY: { label: "Legendario", border: "border-[#ffd60a]",  glow: "shadow-[0_0_20px_rgba(255,214,10,0.5)]",   badge: "bg-[#2a1c00] text-[#ffd60a]",  text: "text-[#ffd60a]",  glowRgb: "255,214,10"   },
  MYTHIC:    { label: "Mítico",     border: "border-[#ff006e]",  glow: "shadow-[0_0_22px_rgba(255,0,110,0.55)]",   badge: "bg-[#2a0015] text-[#ff006e]",  text: "text-[#ff006e]",  glowRgb: "255,0,110"    },
};
    ...
    >
      ...
      >
        ...
        <div
          ...
          style={{ background: `rgba(${rCfg.glowRgb},0.6)` }}
        />
        ...
        <div className="flex items-start gap-4 p-5 pb-3">
          ...
          >
            {isEmoji
              ? myth.art.portrait
              : <img src={myth.art.portrait} alt={myth.name} className="w-full h-full object-contain rounded-xl" />
            }
          </div>
          ...
          <div className="flex-1 min-w-0 pt-1">
            ...
            <div className="flex flex-wrap gap-1.5 mt-2">
              {myth.affinities.map(a => <AffinityBadge key={a} affinity={a} />)}
            </div>
          </div>
        ...
        <div className="px-5 pb-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[#5a6a85] text-xs">Total <span className="text-white font-bold">{total}</span></span>
          </div>
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: `radial-gradient(ellipse at 50% 0%, rgba(${rCfg.glowRgb},0.15) 0%, transparent 70%)` }}
      />
      ...
      {/* Rarity */}
      <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${rCfg.badge}`}>
        {rCfg.label}
      </span>
    </button>
  );
...
export default function MythsPage() {
  ...
  useEffect(() => {
    ...
    (api as any).dex()
      ...
      .catch(() => setError("Error al cargar el Míticadex"))
  ...
  return (
    <div className="h-screen w-screen overflow-hidden flex flex-col" style={{ background: "#070b14", color: "#fff" }}>

      {/* ── Header ── */}
      <div className="shrink-0 px-6 pt-5 pb-3 border-b border-[#1e2d45]">
        <div className="flex items-center justify-between mb-3">
          <div>
            <h1 className="text-xl font-bold tracking-tight">Míticadex</h1>
            <p className="text-[#5a6a85] text-xs mt-0.5">
              {loading ? "Cargando…" : `${filtered.length} de ${creatures.length} criaturas`}
            </p>
          </div>
          <input
            type="text"
