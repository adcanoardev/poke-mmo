...
export default function SantuariosPage() {
    ...
    useEffect(() => { api.gyms().then(setSanctums); }, []);

    async function handleChallenge(id: number) {
        setError(""); setResult(null); setLoading(id);
        try {
            const res = await api.challengeGym(id);
            setResult(res);
            setSanctums(await api.gyms());
        } catch (e: any) {
            setError(e.message);
        } finally {
            setLoading(null);
    ...
    return (
        <Layout sidebar={<TrainerSidebar />}>
            <div className="flex-shrink-0 px-6 py-4 border-b border-border flex items-center justify-between">
                ...
                {result && (
                    <div className={`px-4 py-1.5 rounded-xl border font-display font-bold text-sm
    ...
    <div className="absolute inset-0 bg-bg/80 pointer-events-none" />
                {sanctums.map((s: any) => (
                    <div key={s.id}
                        ...
                        {s.unlocked && !s.earned && (
                            <button onClick={() => handleChallenge(s.id)}
